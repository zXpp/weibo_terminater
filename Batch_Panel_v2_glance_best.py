...
#print(ppt_diago)
	...
	labels=map(lambda x:args["_outprefix"] if Num == 1 else os.path.basename(x).rsplit('.')[0],args["_PPTnames"])
	...
	try:
		for pptname in args["_PPTnames"]:
			...
			label=labels[args["_PPTnames"].index(pptname)]
			newtitle=newtitles[args["_PPTnames"].index(pptname)]
		 ...
		 #cmd=" ".join([r"C:\Windows\system32\wscript.exe "+os.path.join(pyrealpath,r"guidata\Tmp\PPT_Panel.vbs"),vbpptname,imageDir,vbout,str(pdfout)])
			...
			if os.path.exists(tmp) and os.path.exists(tmp2) and Nextflag:#710cunzai
				renamePngs(tmp)
				renamePngs(tmp2)
				relativepath=["".join([r"..","\\",str(width),"\\",i]) for i in os.listdir(tmp)]
				relativerawpath=["".join([r"..\Slides","\\",k]) for k in os.listdir(tmp2)]
				outdir_web=os.path.join(raw, label,collectname)
				...
				pdfname,lpngname,htmlname,xmlname,rlpngname,rawhtmlname,rawxmlname=[os.path.join(outdir_web,label+name) for name in mergename_Sets]
				if os.path.isfile("".join([raw,"\\",label,"\\",label,r".pdf"])):
					shutil.move("".join([raw,"\\",label,"\\",label,r".pdf"]),outdir_web)
				try:
					...
					if args["_raw"]:#pngs2xml(relativepath,xmlname,newtitle)
						...
						if 0 in args["_outFormat"]:
							png2pngs(tmp2,rlpngname)#Imgarr,lpng=
